    ...
    def analyze_video(self, video_path, interval_sec=3, save_crops=True):
        ...
        while True:
            ...
            if frame_idx % frame_interval == 0:
                frame_folder = f"frame_{analyzed_frame_idx + 1}"
                frame_dir = os.path.join(output_dir, frame_folder)
                os.makedirs(frame_dir, exist_ok=True)

                frame_name = f"frame_{analyzed_frame_idx:03d}.jpg"
                temp_path = os.path.join(frame_dir, frame_name)
                cv2.imwrite(temp_path, frame)

                print(f"[INFO] Menganalisis frame ke-{frame_idx} (detik ke-{round(frame_idx / fps, 2)})...")
                ...
                if result:
                    ...
                    result['frame_time_sec'] = round(frame_idx / fps, 2)
